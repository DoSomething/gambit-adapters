...
async function getUserIdBySlackUserId(slackUserId, client) {
  ...
  const slackApiResponse = await client.users.info(slackUserId);
...
function parseOutboundMessage(res) {
  ...
  const { text } = res.body.data.messages.outbound
...
async function reply(message, client) {
  ...
  try {
    // TODO: Define variables in this function instead of modifying our message properties.
    helpers.message.parseMessage(message);

    const userId = await getUserIdBySlackUserId(message.user, client);

    // TODO: Refactor as Slack button instead of sending broadcast command.
    if (message.command === 'broadcast') {
      ...
      const gambitResponse = await gambit.postBroadcastMessage(userId, message.broadcastId);
    ...
    const gambitResponse = await gambit.postMemberMessage(userId, message.text, message.mediaUrl);
    ...
    return await sendMessage(channel, reply.text);
  } catch (error) {
